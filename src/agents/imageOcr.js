...
// ── userPhone added to function signature ─────────────────────────────────────
const imageOcrAgent = async ({ base64Image, mimeType, threadId, filename, userPhone }) => {
  console.log(`[ImageOCR] Processing "${filename}" for thread: ${threadId} | phone: ${userPhone}`);
  ...
  try {
    ...
    console.warn(`[ImageOCR] Cloudinary failed: ${err.message}`);
  }
  ...
  // Step 3: Save report — include user_phone so it's linked to the user
  const saved = await saveReport({
    thread_id:      threadId,
    user_phone:     userPhone || null,    // ← LINKED TO USER
    filename:       filename || "upload",
    extracted_text: extractedText,
    mime_type:      mimeType,
    image_url:      imageUrl,
  ...
  // Step 4b: OCR result as assistant reply (confirmation only, no extracted text)
  await appendMessageToSession(threadId, {
    role:      "assistant",
    type:      "ocr_result",
    content:   `✅ Your medical report has been saved securely. The doctor will review your report soon. Thank you for uploading!`,
    report_id: saved._id.toString(),
    timestamp: new Date(),
  });
  ...
  return {
    ...
    imageUrl,
    extractedText,
    message:       "Medical report uploaded and stored in MongoDB.",
  };
};
